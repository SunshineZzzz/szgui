...
namespace sz_gui
{
	namespace gl
	{
		Geometry::Geometry(size_t posCapacity, size_t colorOrUVCapacity,
			size_t indicesCapacity, bool useColor)
		{
			m_useColor = useColor;
			m_posCapacity = posCapacity * sizeof(float);
			m_colorOrUVCapacity = colorOrUVCapacity * sizeof(float);
			m_indicesCapacity = indicesCapacity * sizeof(uint32_t);
